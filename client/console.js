...
async function setup() {
  if (conf.global()["active-mode"] == "local") {
    ...
    provider = new HDWalletProvider(
      conf.context()["user"]["mnemonic"],
      conf.context()["provider"]["gw"],
      0,
      2
    );
  }
...
async function run_cli() {
  cli
    ...
    .action(async (alias) => {
      ...
      fs.writeFileSync(
        homedir + "/.memo/private_key_" + alias,
  ...
  cli
    ...
    .action(async (alias, msg) => {
      await setup();
      rv = await memo.methods.getUserKeyByAlias(alias).call({ from: source });
      const buf = Buffer.from(rv.slice(2), "hex");
      pk_dec = buf.toString("ascii");
      encrypted = encrypt(pk_dec, msg);
      await memo.methods.sendMemoToAlias(alias, encrypted).send({
        from: source,
        gas: "1000000",
      });
      console.log(
    ...
    .action(async (arg) => {
      ...
      if (arg == "all") {
        ...
        for (i = 0; i < msgs_count; i++) {
          ...
          alias = await memo.methods
            .getUserAlias(rv.source)
            .call({ from: source });
          console.log(
            "index: " +
              i +
  ...
  cli
    ...
    .action(async (arg) => {
      ...
      if (arg != null) {
        defaults = JSON.parse(
          fs.readFileSync("client/user-config.json", "utf8")
        )[arg];
      ...
      try {
        ...
        console.log(
          "failed enrolling user: " + err.data[Object.keys(err.data)[0]].reason
        );
      }
    });
